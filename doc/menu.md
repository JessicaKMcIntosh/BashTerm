...
The interface for the library is the function `term::printf`.
...
For example the options `clear|debug` makes the menu print debug information
and clear the screen before the menu is drawn.
...
Format: `( "Key|Return|Text" )`.

* **Key** -
  The optional key the user should press.
...
* **Return** -
  The optional return code. The default is the positional number.
  If a menu item is completely blank then a blank line is printed.

* **Text** -
  The text to print for the menu item. Required!
...
If you need more than 250 menu items consider
